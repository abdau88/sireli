...
function CurrentProjectID() {
	...
	return "{6F3C73F1-7D22-4D79-93D1-87C709939CCD}";
}
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setCurrentUserName($rs->fields('NIDN')); // Load user name
						$this->setSessionUserID($rs->fields('NIDN')); // Load User ID
	...
	public function loadUserID()
	{
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($rsUser = $UserTableConn->execute($sql)) {
				while (!$rsUser->EOF) {
					$this->addUserID($rsUser->fields('NIDN'));
					$rsUser->moveNext();
				}
				$rsUser->close();
			}
